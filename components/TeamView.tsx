      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative group min-w-[200px] xl:min-w-[300px]">
            ...
            <input type="text" placeholder="Search team..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-6 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all shadow-inner font-bold" />
          </div>
          {isAdmin && (
            <div className="flex items-center space-x-3">
              <button onClick={() => { setInviteEmail(''); setGeneratedInviteLink(''); setIsInviteModalOpen(true); }} className="bg-blue-600 text-white px-8 py-3.5 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-blue-700 shadow-xl shadow-blue-200 transition-all flex items-center space-x-3 shrink-0"><i className="fas fa-paper-plane"></i><span>Invite Agent</span></button>
              <button onClick={handleOpenCreate} className="bg-indigo-600 text-white px-8 py-3.5 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-indigo-700 shadow-xl shadow-indigo-200 transition-all flex items-center space-x-3 shrink-0"><i className="fas fa-plus"></i><span>Add Agent</span></button>
            </div>
          )}
        </div>
      </div>
              ...
              >
                ...
                {isAdmin && (
                  <div className="absolute top-6 right-6 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={(e) => { e.stopPropagation(); handleOpenEdit(user); }} className="w-9 h-9 bg-white text-slate-400 rounded-xl flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-all border border-slate-100 shadow-sm"><i className="fas fa-edit text-xs"></i></button>
                    {!isBroker && user.id !== currentUser.id && (
                      <button onClick={(e) => { e.stopPropagation(); setUserToTrash(user); }} className="w-9 h-9 bg-white text-slate-400 rounded-xl flex items-center justify-center hover:bg-rose-50 hover:text-rose-600 transition-all border border-slate-100 shadow-sm"><i className="fas fa-trash-alt text-xs"></i></button>
                    )}
                  </div>
                )}
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1 mb-6 group-hover:text-blue-500 transition-colors">{isBroker ? 'Broker / Admin' : 'Real Estate Agent'}</p>
              ...
              {filteredUsers.map((user, index) => (
                ...
                >
                  <td className="px-8 py-6">
                    <span className="text-[10px] font-black text-slate-300 group-hover:text-blue-400">{index + 1}</span>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex items-center space-x-4">
                  ...
                  <td className="px-8 py-6">
                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded border transition-all ${user.role === UserRole.BROKER ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}>
                      {user.role}
                    </span>
                  </td>
