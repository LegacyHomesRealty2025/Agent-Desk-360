    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button 
            className="w-11 h-11 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-all shadow-sm"
            title="Page Settings"
          >
          ...
          <div className="relative group min-w-[200px] xl:min-w-[300px]">
            ...
            <input 
              ...
              className="w-full pl-12 pr-6 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all shadow-inner font-bold"
            />
          </div>
          {isAdmin && (
            <div className="flex items-center space-x-3">
              <button 
                ...
                className="bg-blue-600 text-white px-8 py-3.5 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-blue-700 shadow-xl shadow-blue-200 transition-all flex items-center space-x-3 shrink-0"
              >
              ...
              <button 
                ...
                className="bg-indigo-600 text-white px-8 py-3.5 rounded-2xl text-sm font-black uppercase tracking-widest hover:bg-indigo-700 shadow-xl shadow-indigo-200 transition-all flex items-center space-x-3 shrink-0"
              >
          ...
          {filteredUsers.map((user, index) => {
            ...
            return (
              <div 
                ...
                className={`bg-white border rounded-[2.5rem] p-8 hover:shadow-xl transition-all group relative overflow-hidden flex flex-col items-center text-center 
                  hover:bg-blue-50/40 hover:border-blue-500 hover:shadow-blue-100
              ...
              >
                ...
                {isAdmin && (
                  <div className="absolute top-6 right-6 flex space-x-2 pointer-events-auto z-20 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={(e) => { e.stopPropagation(); handleOpenEdit(user); }} className="w-9 h-9 bg-white text-slate-400 rounded-xl flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-all border border-slate-100 shadow-sm">
                      <i className="fas fa-edit text-xs"></i>
                    </button>
                    {!isBroker && user.id !== currentUser.id && (
                      <button onClick={(e) => { e.stopPropagation(); setUserToTrash(user); }} className="w-9 h-9 bg-white text-slate-400 rounded-xl flex items-center justify-center hover:bg-rose-50 hover:text-rose-600 transition-all border border-slate-100 shadow-sm">
                ...
                <div className="relative mb-6">
                  ...
                  <div className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-xl flex items-center justify-center text-white text-[10px] shadow-lg ${isBroker ? 'bg-indigo-600' : 'bg-emerald-500'}`}>
                ...
                <div className="w-full space-y-3 pt-6 border-t border-slate-50 group-hover:border-blue-100 transition-colors">
                  ...
                  <div className="flex items-center space-x-3 text-slate-500 text-sm">
                    <div className="w-8 h-8 bg-slate-50 group-hover:bg-white rounded-lg flex items-center justify-center shrink-0 transition-colors"><i className="fas fa-phone text-[10px]"></i></div>
                    <span className="font-bold group-hover:text-blue-600">{user.phone || 'No phone'}</span>
                  </div>
                  <div className="flex items-center space-x-3 text-slate-500 text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredUsers.map((user, index) => (
                <tr 
                  ...
                  onDragOver={(e) => handleDragOver(e, user.id, user.role)}
                  onDrop={(e) => handleDrop(e, user.id, user.role)}
                  className={`hover:bg-blue-50 transition-all group ${isAdmin && user.role !== UserRole.BROKER ? 'cursor-move' : ''} ${draggedUserId === user.id ? 'opacity-30' : ''} ${dragOverUserId === user.id ? 'bg-indigo-50 border-l-4 border-indigo-500' : ''}`}
                >
                  <td className="px-8 py-6">
                    <span className="text-[10px] font-black text-slate-300 group-hover:text-blue-300">{index + 1}</span>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex items-center space-x-4">
                      ...
                      <p className={`text-sm font-black transition-colors ${user.role === UserRole.BROKER ? 'text-indigo-600' : 'text-slate-800 group-hover:text-blue-700'}`}>{user.firstName} {user.lastName}</p>
                    </div>
                  </td>
                  <td className="px-8 py-6 font-medium text-slate-500 group-hover:text-blue-600 transition-colors">{user.email}</td>
                  <td className="px-8 py-6 font-bold text-slate-700 group-hover:text-blue-700 transition-colors">{user.phone || 'N/A'}</td>
                  <td className="px-8 py-6 font-black text-slate-800 group-hover:text-blue-800 transition-colors">{user.licenseNumber || 'PENDING'}</td>
                  <td className="px-8 py-6">
                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded border transition-all ${user.role === UserRole.BROKER ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100 group-hover:bg-blue-100 group-hover:text-blue-700 group-hover:border-blue-200'}`}>
                      {user.role}
                    </span>
                  </td>
                  <td className="px-8 py-6 text-right">
                    <div className="flex items-center justify-end space-x-2">
                       <button onClick={() => handleOpenEdit(user)} className="w-9 h-9 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center hover:bg-indigo-600 hover:text-white transition-all shadow-sm"><i className="fas fa-pencil-alt text-xs"></i></button>
                       {user.id !== currentUser.id && isAdmin && user.role !== UserRole.BROKER && (
                         <button onClick={() => setUserToTrash(user)} className="w-9 h-9 rounded-xl bg-rose-50 text-rose-600 flex items-center justify-center hover:bg-rose-600 hover:text-white transition-all shadow-sm"><i className="fas fa-trash-can text-xs"></i></button>
                       )}
                    </div>
                  </td>
                </tr>
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl border border-slate-200 w-full max-w-md p-8 relative z-10 animate-in zoom-in-95 duration-200 text-[12px]">
            <div className="flex items-center space-x-4 mb-6 text-rose-600">
              <div className="w-14 h-14 bg-rose-50 rounded-2xl flex items-center justify-center text-2xl shadow-sm"><i className="fas fa-trash-can"></i></div>
              <h3 className="text-xl font-black text-slate-800 tracking-tight">Deactivate Agent?</h3>
            </div>
            <p className="text-slate-600 mb-8 text-base font-semibold leading-relaxed">Are you sure you want to move <span className="text-slate-900 font-black">{userToTrash.firstName} {userToTrash.lastName}</span> to the trash bin? Suspended members cannot log in.</p>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 w-full max-w-4xl p-10 relative z-10 animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[95vh] scrollbar-hide text-[12px]">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-2xl font-black text-slate-800 tracking-tight">{editingUser ? 'Edit Agent Profile' : 'Add New Agent'}</h3>
              <button onClick={() => setIsModalOpen(false)} className="w-12 h-12 flex items-center justify-center rounded-2xl text-slate-400 transition-all hover:bg-rose-50 hover:text-rose-600"><i className="fas fa-times text-xl"></i></button>
            </div>
            {!manualSetupLink ? (
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="pt-6 flex space-x-4">
                  ...
                  <button type="submit" disabled={isProcessing} className="flex-1 py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-indigo-100 hover:bg-indigo-700">{editingUser ? 'Save Profile' : 'Generate Setup Link'}</button>
                </div>
              </form>
            ) : (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-indigo-50 border border-indigo-100 p-8 rounded-[2rem] flex flex-col items-center text-center">
                   ...
                   <h4 className="text-xl font-black text-slate-800 mb-2">Agent Record Saved</h4>
                   <p className="text-sm text-slate-500 font-medium mb-8">Share the setup link below with the new team member.</p>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
           ...
           <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 w-full max-w-lg p-10 relative z-10 animate-in zoom-in-95 duration-200 text-[12px]">
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => setIsInviteModalOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-xl text-slate-400 transition-all hover:bg-rose-50 hover:text-rose-600"><i className="fas fa-times text-lg"></i></button>
              </div>
              {!generatedInviteLink ? (
                <form onSubmit={handleSendInvite} className="space-y-6">
                  <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Agent Email Address</label><input required type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 font-bold outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500" placeholder="agent@email.com" /></div>
                  <div className="space-y-2">
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="bg-emerald-50 border border-emerald-100 p-6 rounded-2xl flex items-center space-x-4">
                    ...
                    <div><p className="text-emerald-900 font-black text-sm">Invitation Prepared</p><p className="text-emerald-700 font-medium">Share the link with {inviteEmail}.</p></div>
                  </div>
                  <div className="space-y-2">
