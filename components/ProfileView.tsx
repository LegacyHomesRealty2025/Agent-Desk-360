...
const ProfileView: React.FC<ProfileViewProps> = ({ user, brokerage, onUpdate, isDarkMode, toggleDarkMode }) => {
  ...
  useEffect(() => {
    // Ensure license number has the prefix if it doesn't already
    const displayUser = { ...user };
    if (displayUser.licenseNumber && !displayUser.licenseNumber.startsWith('DRE Lic# ')) {
      displayUser.licenseNumber = `DRE Lic# ${displayUser.licenseNumber}`;
    } else if (!displayUser.licenseNumber) {
      displayUser.licenseNumber = 'DRE Lic# ';
    }
      ...
      <div className="relative mb-12">
        ...
        <div className="absolute -bottom-12 left-12 flex items-end space-x-8">
          <div className="relative group">
            ...
            <div className="absolute -bottom-2 -right-2 w-10 h-10 bg-emerald-500 rounded-2xl flex items-center justify-center text-white border-4 border-white shadow-lg z-30">
          ...
          <div className="pb-14 space-y-2">
            ...
            <div className="flex items-center space-x-6 text-indigo-100/80 font-bold text-sm">
              ...
              <span className="flex items-center"><i className="fas fa-id-card mr-2 text-indigo-300"></i>{formData.licenseNumber}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-8 mt-24">
        ...
        <div className="flex justify-center pt-12">
           <button 
              ...
              disabled={isProcessing}
              className={`px-20 py-8 ${isSaved ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-rose-600 hover:bg-rose-700 shadow-[0_20px_50px_rgba(225,29,72,0.3)]'} text-white rounded-[2.5rem] font-black uppercase tracking-[0.5em] text-sm transition-all active:scale-[0.98] disabled:opacity-50 flex items-center space-x-6`}
            >
